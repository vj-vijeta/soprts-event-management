...
export class NavbarComponent implements OnInit {
  userName: any
  userId: any
  accSettingshow: boolean = true
  @Input() settingShow: any = true
  ...
  constructor(private routing: Router, private service: AuthenticateService, private http: HttpClient) {
    ...
    this.service.userDetails(JSON.parse(localStorage.getItem('userId') || ''), this.user)
    ...
    this.service.logService(this.userId, this.user)
      .subscribe((result: any) => {
        if (result) {
  ...
  logOk() {
    if (localStorage.getItem('userId')) {
      ...
      this.service.logoutService(this.userId, this.user)      
        .subscribe((result: any) => {
          if (result) {
            this.Ok = "Ok"
